...
int findOtsu2 (THIS_UINT8 *img8bit, int nVox)
{
    int lHisto[256];
...
void findOtsu3 (THIS_UINT8 *img8bit, int nVox, int* lo, int* hi)
{
    int lHisto[256];
    *lo = 85;
    *hi = 170;
...
void findOtsu4 (THIS_UINT8 *img8bit, int nVox, int* lo, int* mid, int* hi)
{
    int lHisto[256];
    *lo = 64;
    *mid = 128;
...
void applyOtsu2 (THIS_UINT8 *img8bit, int nVox)
{
    if (nVox < 1) return;
...
void applyOtsu3 (THIS_UINT8 *img8bit, int nVox)
{
    if (nVox < 1) return;
    int lo, hi;
...
void applyOtsu4 (THIS_UINT8 *img8bit, int nVox)
{
    if (nVox < 1) return;
    int lo, mid, hi;
...
void countClusterSize (THIS_UINT8 *lImg, THIS_INT64 *lClusterBuff, int lXi, int lYi, int lZi,  THIS_UINT8 lClusterValue)
{
    ...
    THIS_INT64 *lQra = (THIS_INT64 *) malloc(lXYZ * sizeof(THIS_INT64));
    ...
    int const nSearch = 6;
    THIS_INT64 *searchArray = (THIS_INT64 *) malloc(nSearch * sizeof(THIS_INT64));
        ...
        if (lClusterBuff[v] == kUndeterminedSize) {
            ...
            do {
                for (int s = 0; s < nSearch; s++) {//search each neighbor to see if it is part of this cluster
                    THIS_INT64 vx = lQra[lQCurr] + searchArray[s];
...
void fillBubbles (THIS_UINT8 *lImg, int lXi, int lYi, int lZi)
...
void preserveLargestCluster (THIS_UINT8 *lImg, int lXi, int lYi, int lZi, int lClusterValue, THIS_UINT8 ValueForSmallClusters)
{
    if ((lXi < 5) or (lYi < 5) or (lZi < 1)) return;
    int lXYZ,lX;
    THIS_INT64 lC;
    ...
    THIS_INT64 *lTemp = (THIS_INT64 *) malloc(lXYZ * sizeof(THIS_INT64));
...
void dilate (THIS_UINT8 *lImg, int lXi, int lYi, int lZi, int lCycles, int lChange)
...
//step is repeated  for lCycles
{
    ...
    lXYZ =lXY*lZi;
    THIS_UINT8 *lTemp = (THIS_UINT8 *) malloc(lXYZ);
...
void dilateSphere (THIS_UINT8 *lImg, int lXi, int lYi, int lZi, float lVoxDistance, int lChange)
...
//OUTPUT: Eroded/Dilated Img
{
    ...
    THIS_INT64 *lSearch = (THIS_INT64 *) malloc( ((lDxI *2)+1)*((lDxI *2)+1)*((lDxI *2)+1) *sizeof(THIS_INT64) );
    int lVoxOK = 0;
...
void smoothFWHM2Vox (THIS_UINT8 *lImg, int lXi, int lYi, int lZi)
{
...
//void countSurvivors (THIS_UINT8 *img8bit, int lXYZ)
//{
//    int count = 0;
//    for (int i = 0; i < lXYZ; i++)
//        if (img8bit[i] != 0) count++;
//    NSLog(@" %d survivors", count);
//}
